...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run station-stacking v9 year-split training.")
    ...
    parser.add_argument("--output-dir", type=Path, default=REPO_ROOT / "data/calibration/station_stacking_v9")
    parser.add_argument(
        "--climatology-normals",
        type=Path,
...
def main() -> None:
    ...
    for station in stations:
        config = StationStackingConfig(
            ...
            stack_optuna_trials=args.optuna_trials,
            optuna_startup_trials=args.startup_trials,
            stack_optuna_startup_trials=args.startup_trials,
        ...
        if not args.skip_export:
            exported = export_station_model_weights(
                ...
                optuna_metric=OPTUNA_METRIC,
                target_mode=TARGET_MODE,
                source_pipeline="scripts/run_station_stacking_v9.py",
            )
