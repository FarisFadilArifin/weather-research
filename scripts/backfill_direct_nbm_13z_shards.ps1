...
function Save-Status {
    param([object[]]$Shards)
    $Shards |
        Select-Object shard_id, start_date, end_date, state, pid, exit_code, started_at, finished_at, cache_dir, stdout_log, stderr_log |
...
function Get-ShardCacheStats {
    param([object]$Shard)
    $cachePath = Join-Path $Shard.cache_dir "direct_nbm_0h_cache.csv"
    $expectedRows = Get-ShardExpectedRows -Shard $Shard
    if (-not (Test-Path $cachePath)) {
        return [pscustomobject]@{ rows = 0; ok_rows = 0; non_ok_rows = 0; expected_rows = $expectedRows; complete = $false }
    }
    try {
...
if ($PrefetchWorkers -lt 1) {
    throw "PrefetchWorkers must be >= 1"
}
$env:WEATHER_RESEARCH_NBM_PREFETCH_WORKERS = [string]$PrefetchWorkers

$periods = @(New-ShardPeriods -Start $start -End $end -Months $ShardMonths)
$shards = @()
foreach ($period in $periods) {
    $shardId = "nbm13z_{0}_{1}" -f ($period.start_date -replace "-", ""), ($period.end_date -replace "-", "")
    $cacheDir = Join-Path $DataRoot "direct_nbm_13z_live_safe_$shardId"
