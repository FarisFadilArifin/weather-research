param(
    ...
    [string[]]$FeatureFields = @("temp_k_2m", "dewpoint_k_2m", "relative_humidity_pct_2m", "precip_mm_1h", "cloud_cover_pct"),
    [string]$TimingMode = "same_day_11am_live_safe",
    [string]$LogLabel = "direct_nwp"
)
...
$LogRoot = Join-Path $ProjectRoot "logs\direct_nwp_shards_$LogLabel"
...
if (-not (Test-Path $Python)) { throw "Missing project Python interpreter: $Python" }
...
function Get-ShardCacheStats {
    ...
    if (-not (Test-Path $cachePath)) {
        return [pscustomobject]@{ rows = 0; ok_rows = 0; weather_ok_rows = 0; expected_rows = $expectedRows; complete = $false }
    }
    try { $rows = @(Import-Csv -Path $cachePath) }
    catch { return [pscustomobject]@{ rows = 0; ok_rows = 0; weather_ok_rows = 0; expected_rows = $expectedRows; complete = $false } }
    $modelRows = @($rows | Where-Object { $_.provider -eq $Shard.model -and $_.model -eq $Shard.model })
    $okRows = @($modelRows | Where-Object { $_.fetch_status -eq "ok" }).Count
    $weatherOkRows = @($modelRows | Where-Object { $_.fetch_status -eq "ok" -and $_.weather_features_included -eq "True" }).Count
    return [pscustomobject]@{
        rows = $modelRows.Count
        ok_rows = $okRows
        weather_ok_rows = $weatherOkRows
        expected_rows = $expectedRows
        complete = (($modelRows.Count -ge $expectedRows) -and ($okRows -eq 0 -or $weatherOkRows -eq $okRows))
    }
}
...
foreach ($model in $Models) {
    ...
    foreach ($period in $periods) {
        $shardId = "{0}_{1}_{2}" -f $modelLower, ($period.start_date -replace "-", ""), ($period.end_date -replace "-", "")
        $cacheDir = Join-Path $DataRoot "sdk_11am_live_safe_direct_$shardId"
...
Write-EventLine "created manifest with $($shards.Count) shards; max_parallel=$MaxParallel shard_months=$ShardMonths timing_mode=$TimingMode models=$($Models -join ',') feature_fields=$($FeatureFields -join ',')"
    ...
    foreach ($shard in @($shards | Where-Object { $_.state -eq "running" })) {
        ...
        if ($proc.HasExited) {
            ...
            if (($exitCode -eq 0) -or $stats.complete) {
                $shard.state = "done"
                if ($stats.complete -and ($exitCode -ne 0)) { $shard.exit_code = "cache_complete" }
                Write-EventLine "done $($shard.shard_id) exit=$($shard.exit_code) rows=$($stats.rows) ok=$($stats.ok_rows) weather_ok=$($stats.weather_ok_rows)"
            }
            else {
                $shard.state = "failed"
                Write-EventLine "failed $($shard.shard_id) exit=$exitCode rows=$($stats.rows) ok=$($stats.ok_rows) weather_ok=$($stats.weather_ok_rows)"
            }
            $shard.process = $null
        }
    }
    ...
    if ($slots -gt 0) {
        foreach ($shard in @($shards | Where-Object { $_.state -eq "pending" } | Select-Object -First $slots)) {
            ...
            if ($stats.complete) {
                ...
                $shard.finished_at = (Get-Date).ToString("o")
                Write-EventLine "skipped $($shard.shard_id) cache_complete rows=$($stats.rows) ok=$($stats.ok_rows) weather_ok=$($stats.weather_ok_rows)"
                continue
            }
            $args = @(
                "-m", "src.backfill_direct_nwp",
