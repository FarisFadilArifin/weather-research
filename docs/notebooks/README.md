...
The active training entry point is the non-versioned
[`station_training_baseline`](../station-training/README.md). It uses one
self-contained notebook per station; KDAL is the first reference station.

All `station_stacking_v*` directories are preserved historical research. Use
them for evidence and comparison, not as the starting point for a new station.
...
datasets, exports, and model artifacts, use `notebooks/experiments/exploratory_analysis/exploratory_analysis.ipynb`.

The earlier station-stacking v2 notebooks remain available as historical
same-day 11 AM research:
...
notebooks/experiments/station_stacking_v2/stacking_KDAL_v2.ipynb
notebooks/experiments/station_stacking_v2/stacking_KHOU_v2.ipynb
notebooks/experiments/station_stacking_v2/stacking_KLAX_v2.ipynb
notebooks/experiments/station_stacking_v2/stacking_KLGA_v2.ipynb
notebooks/experiments/station_stacking_v2/stacking_KMIA_v2.ipynb
notebooks/experiments/station_stacking_v2/stacking_KORD_v2.ipynb
notebooks/experiments/station_stacking_v2/stacking_KSEA_v2.ipynb
```
...
```text
notebooks/experiments/station_stacking_v4/stacking_KATL_v4.ipynb
notebooks/experiments/station_stacking_v4/stacking_KAUS_v4.ipynb
notebooks/experiments/station_stacking_v4/stacking_KDAL_v4.ipynb
notebooks/experiments/station_stacking_v4/stacking_KHOU_v4.ipynb
notebooks/experiments/station_stacking_v4/stacking_KLAX_v4.ipynb
notebooks/experiments/station_stacking_v4/stacking_KLGA_v4.ipynb
notebooks/experiments/station_stacking_v4/stacking_KMIA_v4.ipynb
notebooks/experiments/station_stacking_v4/stacking_KORD_v4.ipynb
notebooks/experiments/station_stacking_v4/stacking_KSEA_v4.ipynb
```

These notebooks write artifacts to `data/calibration/station_stacking_v4`.

`notebooks/experiments/station_stacking_v5/` uses the same v4 feature engineering and 100/50 Optuna trial counts, but tunes Optuna against MAE instead of RMSE. It writes artifacts to `data/calibration/station_stacking_v5`.

## V6 Feature-Input Experiment

`notebooks/experiments/station_stacking_v6/` starts from the source-owned v5 feature engineering block, keeps the MAE Optuna setup, adds durable per-station Optuna SQLite storage, and includes the 11 AM observation trend columns when those cache fields are populated.
...
`notebooks/experiments/station_stacking_v14/` is a controlled successor to v13. It keeps the v11 remaining-warmup, Huber, and ridge-stack lineage, computes weather aggregates from the enriched forecast caches, then trains only on the v11 feature base plus a curated aggregate weather allowlist that passes coverage checks. Artifacts are written to `data/calibration/station_stacking_v14`.
...
- `notebooks/experiments/station_stacking_v19/stacking_KATL_v19_b.ipynb`
- `notebooks/experiments/station_stacking_v19/stacking_KDAL_v19_b.ipynb`
- `notebooks/experiments/station_stacking_v19/stacking_KATL_v19_c.ipynb`
- `notebooks/experiments/station_stacking_v19/stacking_KDAL_v19_c.ipynb`
