...
# Source-owned v10 contract markers:
# feature_version="v10"
...
def _notebook(station_id: str) -> dict:
    ...
    replacements = [
        ("Station Stacking v9", "Station Stacking v10"),
        ('feature_version="v9"', 'feature_version="v10"'),
        (
            'MODEL_VERSION = "station_high_regressor_v9_remaining_warmup"',
            'MODEL_VERSION = "station_high_regressor_v10_catboost_huber"',
        ),
        ("STACK_OPTUNA_TRIALS = 30", "STACK_OPTUNA_TRIALS = 0"),
        ("STACK_OPTUNA_STARTUP_TRIALS = 15", "STACK_OPTUNA_STARTUP_TRIALS = 0"),
        (
            'output_dir=PROJECT_ROOT / "data" / "calibration" / "station_stacking_v9"',
            'output_dir=PROJECT_ROOT / "data" / "calibration" / "station_stacking_v10"',
        ),
        (
        ...
        ("V9_DROPPED_FEATURE_COLUMNS", "V10_DROPPED_FEATURE_COLUMNS"),
        ("V9_FEATURE_COLUMNS", "V10_FEATURE_COLUMNS"),
        ("V9 Contract", "V10 Contract"),
        ("V9 Feature Coverage", "V10 Feature Coverage"),
        ("v9_feature_coverage", "v10_feature_coverage"),
        ("## V9", "## V10"),
        ("`feature_version=\\\"v9\\\"`", "`feature_version=\\\"v10\\\"`"),
        (
            ...
            "This version keeps the v9 live-safe GFS/HRRR/NBM remaining-warmup data contract, trains only CatBoost with tuned Huber loss, disables ridge stacking, and writes artifacts to `data/calibration/station_stacking_v10`.",
        ),
        (
            '`feature_version="v10"` keeps the v8 remaining-warmup feature set, adds leakage-safe 10-year calendar-day climatology features, and uses `target_mode="remaining_warmup"` so base learners fit `actual_high_f - observed_high_temp_through_as_of_f` before converting back to `predicted_high_f`.',
            '`feature_version="v10"` keeps the v9 feature contract, uses `target_mode="remaining_warmup"`, and restricts the experiment to CatBoost with tuned Huber loss. Ridge stacking is disabled so the final exported model is the CatBoost base learner.',
        ),
        (
            '    target_mode="remaining_warmup",\n',
            '    target_mode="remaining_warmup",\n    base_model_methods=("catboost",),\n    stack_enabled=False,\n',
        ...
        ('source_pipeline="notebooks/experiments/station_stacking_v9"', 'source_pipeline="notebooks/experiments/station_stacking_v10"'),
        (
            "missing_packages = missing_model_dependencies()",
            'missing_packages = missing_model_dependencies(("catboost",))',
        ),
    ]
...
def main() -> None:
    ...
    for station in TARGET_STATIONS:
        ...
        path = out_dir / f"stacking_{station}_v10.ipynb"
