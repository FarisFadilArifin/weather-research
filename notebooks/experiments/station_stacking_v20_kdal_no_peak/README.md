...
```powershell
.\.venv\Scripts\python.exe notebooks\experiments\station_stacking_v20_kdal_no_peak\generate_station_notebook.py
```
...
To enrich and materialize the KDAL feature tail through July 29 with isolated
one-day shards, 12 top-level workers, and three nested forecast workers, run:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\run_v20_kdal_no_peak_enrichment.ps1 `
    -StartDate 2026-07-21 -EndDate 2026-07-29
```

The runner fetches full-weather GFS, HRRR, and direct NBM inputs plus 11 AM
observations and Wunderground targets. It resumes complete shard caches, retries
incomplete shards once, then rebuilds `KDAL_features.csv` and requires 100%
coverage of the requested tail before succeeding.
